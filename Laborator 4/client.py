...
############################### EXERCISE 1 ###############################
    def recv_handler(self,stop_event):
        while True:
            ...
            if len(data) > 0:
                if data[4:6] == "01":
                    self.set_dtc1_state(data[6:])
                elif data[4:6] == "02":
                    self.set_dtc2_state(data[6:])
                elif data[4:6] == "03":
                    self.set_dtc3_state(data[6:])
                elif data[4:6] == "04":
                    self.set_dtc4_state(data[6:])
    ...
    def diag(self):
        self.sock.send('0x3E01'.encode())
        self.diagMode.setEnabled(False)
        self.diagMode_off.setEnabled(True)

    def stop_diag(self):
        self.sock.send("0x3E00".encode())
        self.diagMode.setEnabled(True)
        self.diagMode_off.setEnabled(False)
...
############################### EXERCISE 3 ###############################        
    ...
    def get_dtc_state(self,dtc_string):
        if self.diagMode_off.isEnabled():
...
############################### EXERCISE 4 ###############################
    ...
    def set_led0_label(self,data_recv):
        ...
        self.sock.send(data_recv.encode())
    
    def set_led0_flags(self):
        if self.diagMode_off.isEnabled():
            message = "0x2E01"

            if self.set_led0.text() == "Set LED0":
                message += '1'
                self.set_led0.setText("Unset LED0")
            else:
                message += '0'
                self.set_led0.setText('Set LED0')
    ...
    def set_led1_flags(self):
        if self.diagMode_off.isEnabled():
            ...
            if self.set_led1.text() == "Set LED1":
                message += '1'
                self.set_led1.setText("Unset LED1")
            else:
                message += '0'
                self.set_led1.setText('Set LED1')
    ...
    def set_led2_flags(self):
        if self.diagMode_off.isEnabled():
            ...
            if self.set_led2.text() == "Set LED2":
                message += '1'
                self.set_led2.setText("Unset LED2")
            else:
                message += '0'
                self.set_led2.setText('Set LED2')
    ...
    def set_led3_flags(self):
        if self.diagMode_off.isEnabled():
            ...
            if self.set_led3.text() == "Set LED3":
                message += '1'
                self.set_led3.setText("Unset LED3")
            else:
                message += '0'
                self.set_led3.setText('Set LED3')
