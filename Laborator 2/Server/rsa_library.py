...
def encrypt(public_key, hex_number):
    return pow(hex_number, public_key[0])%public_key[1]
    ''' complete with necesarry code '''

############################### EXERCISE 2 ###############################
def decrypt(private_key, encrypted_msg):
    return pow(encrypted_msg, private_key[0])% private_key[1]
    ''' complete with necesarry code '''

############################### EXERCISE 3 ###############################
def low_check(hex_nr):
    nr = '0x' + str(format(hex_nr,'03X').zfill(4))
    if len(nr) >6 or nr[4:]!= ON_low[2:]:
        return False
    return True
    ''' complete with necesarry code '''
...
def number_check(hex_nr):
    ...
    if len(nr) > 6:
        return False
    highB = format(int(nr[0:4],16),'08b')
    lowB = format(int('0x'+nr[4:6],16),'08b')
